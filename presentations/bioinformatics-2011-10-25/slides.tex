...
\frame{
  ...
  Case study: Python MySQLdb library and closed source software

%\colorbox{blue}{\makebox[\linewidth][l]{hi!}}

%\begin{tikzpicture}
%  \draw node[draw=black,fill=black!20,rounded corners,inner sep=1ex,text width=15em] {
...
\node[fancytitle, rounded corners] at (box1.east) {GPL};

\node [mybox,below=8pt of box1.south] (box2) {%
    \begin{minipage}[t!]{0.8\linewidth}
      MySQLdb
    \end{minipage}
    };
\node[fancytitle, rounded corners] at (box2.east) {MIT};

\draw [mybox] (box1) -- (box2);

\node [mybox,below=8pt of box2.south] (box3) {%
    \begin{minipage}[t!]{0.8\linewidth}
      libmysql
    \end{minipage}
    };
...
\draw[style=densely dashed] [mybox] (box3) -- (box4);
