...
\date{\sffamily Literature Study, 2009 r\svnrev}
...
examples of CPS transformations in Section~\ref{sec:transformations}.

\paragraph{}

Formal verification of programs is used to prove that a program does not have certain
defects or has certain properties. Traditionally, program verification is done
...
We will do a simplified case study on the setting of Dargaye and Leroy. Where their
...
transformation, a straightforward extension of Plotkin's original transformation,
...
source and target languages of the transformation.
...
will be used instead of named variables.
...
abstraction.
Now $\alpha$-equivalence of terms simply corresponds to syntactic equality.
...
We will use the following notation to distinguish between lifting of
...
Again, we will follow the presentation of \cite{Dargaye-Leroy-07}.
The transformation we will prove correct is a straightforward extension of
...
We will say that a target language term is $\kappa$-closed if it does not
contain any free continuation variables $\kappa_{i}$.
The following lemmas state some important properties of the transformation.
They will be proved in appendix~\ref{sec:proofs}.
...
statement we will prove is given as Lemma~\ref{lem:mainlemma} and generalizes the
...
tools (e.g. a proof assistant).
Reasons for doing so are several.
Firstly, a mechanically checked proof is often more precise and convincing than
a proof on paper.
Secondly, mechanical proofs as are often easier to be reused in many ways, e.g.
...
substitution.
Furthermore, terms with de Bruijn variables are arguably harder to deal with for
...
Furthermore, the intended use of the transformation is in a compiler for
...
``locally nameless'' representation \cite{Leroy-07}, where free variables are
named and bound variables are represented by their de Bruijn index.
...
compositional way.
These mathematical structures, called denotations, describe the behavior of
the language.
...
Many thanks are due to Femke van Raamsdonk and Roel de Vrijer for their
supervision.
Your patience seemed unbounded but must have been tested to the limit.
Heiltje Winterink provided comments on my English writing, thanks for that.
And finally, \emph{muchas gracias} Martina.
...
Lemma~\ref{lem:kappaclosed}). Furthermore, we use the fact $\Uparrow_{x}^{n} \Psi(A) = \Psi(\Uparrow^{n} A)$
...
\item[\sffamily Inductive case $M = \lambda^{m}.M_{1}$]\hfill
  ...
  \begin{eqnarray*} % eqnarray shouldn't be used
