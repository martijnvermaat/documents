...
features two kinds of variables as opposed to one in the source language.
The extra kind in the target language is used for continuations and
...
$\Psi$ on atoms and $[\![\cdot]\!]$ on terms. An atom is a variable $x_{n}$ or
a term of the form $\lambda^{n}.M$.
...
The transformation of $M_{1} \ldots M_{n} \: \kw{then} \: N'$ first evaluates
...
Some properties of the transformation (lemma 3 and 4).
...
\begin{description}
\item[\sffamily Base case $\boldsymbol{M = \lambda^{n}.M_{1}}$]\hfill
...
\item[\sffamily Inductive case $\boldsymbol{M = M_{1}(N_{0}, \ldots, N_{n})}$]\hfill
...
\item[\sffamily Inductive case $\boldsymbol{M = \kw{let} \: M_{1} \: \kw{in} \: M_{2}}$]\hfill
