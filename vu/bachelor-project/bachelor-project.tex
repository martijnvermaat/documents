...
Intro text. We need a generalization of our ultimate goal in order for a proof
by induction to go through.
...
\paragraph{Case $M = \lambda^{n}.M_{1}$}
\begin{quotation}\noindent
  We need to show
  $(\lambda^{0}.\kappa_{0}(\Psi(\lambda^{n}.M_{1}))) (K) \Rightarrow v'$.
  Trivial.
\end{quotation}
\paragraph{Case $M = M_{1}(N_{0}, \ldots, N_{n})$}
\begin{quotation}\noindent
...
\end{quotation}
\paragraph{Case $M = \kw{let} \: M_{1} \: \kw{in} \: M_{2}$}
\begin{quotation}\noindent
  ...
  \begin{equation*}
    ...
    (\Uparrow_{x}^{1} \lambda^{0}.P)) \Rightarrow v' \text{ .}
  \end{equation*}
  is proved by first applying the induction hypothesis to the first premise,
  then to the second premise, and some evaluation calculations.\qedhere
\end{quotation}
\end{proof}
...
\item{Case $M = \kw{let} \: M_{1} \: \kw{in} \: M_{2}$}
