...
% TODO: Cite only works really read?
...
%       => No error! abstractions evaluate to themselves! (check in text)
%                    (investigate why => is defined this way)
% TODO: Check how this compares to Plotkin's proof
% TODO: Discussion and conclusion
% TODO: Finish proof of lemma 3
...
% TODO: explain let binding, also in target language
...
% TODO: No this abstraction just evaluates to itself!
%       (This error is also on the slides)
...
\item[\sffamily Base case $M = \lambda^{n}.M_{1}$]\hfill
  ...
  \end{eqnarray*}
  % TODO: evaluations are always in one step!
...
\end{equation}
by (\ref{eq:thirdpremise}) via lemma~\ref{lem:kappaclosed}.
...
\end{equation}
by (\ref{eq:psiq}) and repeatedly (for $i = 0 \ldots n$) applying the
...
Now, (\ref{eq:faresult}) is proved using the evaluation rule for
...
\begin{equation}\label{eq:letsecondprem} % TODO: renumber equations from 1
