...
\frame{
  ...
  \begin{itemize}
    ...
    \item Automation
    \item Gains insight and intuition
...
(\coqdocprojection{lhs}
\coqdocvariable{$\rho_1$})$^{\coqdocvariable{$\sigma$}}$[$\Box$]$_{\coqdocvariable{p}}$ \coqdockw{with}\coqdoceol
\coqdocindent{3.00em}
\ensuremath{|}
...
\coqdocnoindent
\coqdockw{Definition}
\coqdocdefinition{$\phi_b$} % TODO: here the S(2 n) is not correct
...
\frame{
  ...
  \begin{block}{Selected references}
    \begin{itemize}
    \item Peter Hancock, (Ordinal-theoretic) Proof Theory
    \item J\"org Endrullis et al, Unique Normal Forms in Infinitary
      Weakly Orthogonal Term Rewriting
    \item Terese, Term Rewriting Systems
    \item Jan Willem Klop and Roel de Vrijer, Infinitary Normalization
