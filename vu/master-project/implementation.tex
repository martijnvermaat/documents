...
\coqdocindent{1.00em}
\ensuremath{|} \coqref{Rewriting.Cons}{\coqdocconstructor{Cons}} \coqdocvar{s}
...
In an effort to prevent getting lost in a notational labyrinth, we define
...
Now we turn to embeddings of rewrite sequences.
\begin{singlespace}
\begin{coqdoccode}
\coqdocnoindent
...
\coqref{Rewriting.predtype}{\coqdocdefinition{pred\_type}} \coqdocvariable{$\tau$})
(\coqdocvar{$\rho$} : \coqdocvariable{s}
\coqref{Rewriting.sequence}{$\twoheadrightarrow_\mathcal{R}$}
\coqdocvariable{$\tau$}[\coqdocvariable{i}]$^\textsc{l}$),\coqdoceol
\coqdocindent{9.50em}
\coqdocvariable{$\rho$} \coqref{Rewriting.embed}{$\sqsubseteq$} \coqdocvariable{$\tau$}[\coqdocvariable{i}]$^\textsc{seq}$ \ensuremath{\rightarrow}\coqdoceol
\coqdocindent{9.50em}
\coqref{Rewriting.Cons}{\coqdocconstructor{Cons}} \coqdocvariable{$\rho$}
\coqdocvariable{$\tau$}[\coqdocvariable{i}]$^\textsc{stp}$ \coqref{Rewriting.embed}{$\sqsubseteq$}
\coqdocvariable{$\tau$}\coqdoceol
\coqdocindent{1.00em}
\ensuremath{|}
