...
would implement a vector type inductively, as for example has been done in the
\Coq standard library:
\begin{singlespace}
\begin{coqdoccode}
\coqdocnoindent
\coqdockw{Inductive} \coqdocvar{vector} : \coqdocvar{nat}
\ensuremath{\rightarrow} \coqdockw{Type} :=\coqdoceol
\coqdocindent{1.00em}
\ensuremath{|} \coqdocvar{Vnil} : \coqdocvar{vector} 0\coqdoceol
\coqdocindent{1.00em}
\ensuremath{|} \coqdocvar{Vcons} : \ensuremath{\forall} (\coqdocvar{a}:A)
(\coqdocvar{n}:nat), \coqdocvar{vector} \coqdocvar{n} \ensuremath{\rightarrow}
\coqdocvar{vector} (\coqdocvar{S} \coqdocvar{n}).\coqdoceol
\end{coqdoccode}
\end{singlespace}
% TODO: links to stdlib documentation
