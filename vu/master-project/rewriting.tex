...
% TODO: note that we are explicit in de wording: sequence vs rewrite sequence
...
% TODO: is this equality story correct?
%We use the symbol $\equiv$ for syntactic equality on terms. Sometimes, we
%want to be more explicit as to what this equality amounts to. Proving two
%infinite objects equal is generally done by giving a bisimulation relation
%between them.

Especially on a coinductive domain, we want to be precise about the notions of
equality we employ. We consider terms to be equal if they are
\begin{inparaenum}[(i)]
  \item bisimilar or
  \item syntactically equivalent up to all depths.
\end{inparaenum} According to Proposition~\ref{prop:equalities} it does not
matter ultimately which equality we use.

\begin{definition}%[Bismilarity]
We define the \emph{bisimularity relation} $\sim$ on $Ter_\Sigma^\infty$ by
coinduction:
\begin{compactenum}
  \item
    $x \sim x$ for every variable $x \in X$.
  \item
    For every $f \in \Sigma$, if $s_i \sim t_i$ for all $i \leq
    \#_f$, then $f(s_1, \ldots s_{\#_f}) \sim f(t_1, \ldots, t_{\#_f})$.
\end{compactenum}
...
\begin{compactenum}
  ...
  \item For every $f \in \Sigma$, if $s_i \equpto{d} t_i$ for all $i \leq
    \#_f$, then $f(s_1, \ldots s_{\#_f}) \equpto{d+1} f(t_1, \ldots,
    t_{\#_f})$.
\end{compactenum}
...
% TODO: define d(t,u) and depth of rewrite steps
\begin{definition}%[Cauchy convergence]
  A sequence of terms $(t_\beta)_{\beta < \alpha}$ \emph{(Cauchy-) converges}
  to the term $t$ if $d(t_\beta, t)$ tends to $0$ when $\beta$ approaches
  $\alpha$ from below.
\end{definition}
