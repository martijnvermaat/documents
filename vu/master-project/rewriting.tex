...
%order relation on tree ordinals in Subection~\ref{sub:tree} is by
%\citet{hancock-08}.
...
ordinal.
%From now on, we identify ordinals and their representation as tree
%ordinal.

% some ordinals have no representation as tree ordinal at all
...
% this makes it undecidable to compare an ordinal to 0 or 3
...
\begin{align*}
  \Phi(0)                 &= \nothing \\
  \Phi(\alpha^+)          &= 1 + \Phi(\alpha) \\ %\Phi(\alpha)^? \\
  \Phi(\sqcup_i \alpha_i) &= (\Sigma n \in \mathbb{N}) \; \Phi(\alpha_n)
\end{align*}
\end{definition}

By $1 + A$ we mean the disjoint sum of the unit type $1$ and $A$,
...
An extensional equality on $\Ord$ can now be defined on top of this
order.

\begin{definition}%[Equality on tree ordinals]
...
%First, the extensional \emph{equality} $\alpha \simeq
...
\end{compactenum}
\end{definition}
% maybe use Ter(\Sigma, X) instead of Ter_\Sigma(X)
...
%    t_{\arity{f}})$.
    % ugly solution to prevent line overflow
...
% We restrict ourselves to rewrite rules where $l$ is
...
%also Subsection~\ref{sub:contexts}).
...
%      (i.e.\ if there is no step from $t$).
  ...
  \end{compactenum}
\end{definition}
% klop-de-vrijer-05 uses our UN->> definition for UN^\infty,
...
definition.
% informal definition of descendants and confluence

% maybe cite joerg's thesis
